...
import { WSEventType, type ChatEvent, type OutgoingChatEvent, isChatEvent } from '@platica/shared/src/websockets'
import type { UiMessage } from '@models/message'
import type { CreateMessageDTO } from '@models/message'
...
interface MessageContextValue {
  ...
  // Actions
  loadMessages: (channelId: number, options?: { before?: number }) => Promise<void>
...
export function MessageProvider({ children }: { children: React.ReactNode }) {
  ...
  const { state: { workspace } } = useAppContext()
    ...
    try {
      ...
      if (!success) {
        dispatch({ 
          ...
          payload: new Error('Failed to send message - WebSocket not connected')
        })
      }
    } catch (error) {
      dispatch({ 
  ...
  const getMessageById = useCallback((messageId: number): UiMessage | undefined => {
    return state.byId[messageId]
  }, [state.byId])

  const isLoadingMessages = useCallback((channelId: number): boolean => {
    return !!state.loading.channels[channelId]
  }, [state.loading.channels])

  const messageError = useCallback((channelId: number): Error | null => {
    return state.errors.channels[channelId] || null
  }, [state.errors.channels])

  const hasMoreMessages = useCallback((channelId: number): boolean => {
