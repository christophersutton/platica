...
export function PresenceProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const unsubSync = subscribe(WSEventType.PRESENCE_SYNC, (event: PresenceSyncEvent) => {
      ...
      event.payload.onlineUsers.forEach((userId) => {
        presenceMap[userId] = {
          status: 'online',
          lastSeen: now
        }
  ...
  // Actions
  const setUserStatus = useCallback((userId: number, status: UserPresenceStatus) => {
  ...
  const setUserRoom = useCallback((userId: number, roomId?: number) => {
  ...
  const setCustomStatus = useCallback((userId: number, status?: string) => {
  ...
  const isUserAvailableForChat = useCallback((userId: number) => {
    const presence = state.presenceMap[userId]
    if (!presence) return false
    return presence.status === 'online' // Only available if online and not in room
  }, [state.presenceMap])
