...
> {
  ...
  async getWorkspaceWithMeta(
    workspaceId: number,
    userId?: number
  ): Promise<WorkspaceWithMeta | undefined> {
    const query = `
      SELECT 
        w.*,
    ...
    const result = this.db
      .prepare(query)
      .get(...params) as WorkspaceWithMeta | null;
    return result ? this.deserializeRow(result) : undefined;
  }
  ...
  async addUser(
    ...
    console.log("Checking if user is already a member", workspaceId, userId);
    const existingRole = await this.getMemberRole(workspaceId, userId);
    console.log("existingRole", existingRole);
    if (existingRole) {
      console.log("User is already a member", existingRole);
      return; // User is already a member, silently succeed
    }
    console.log("User is not a member, adding them");
  ...
  async updateUser(
    ...
    if (
      ...
      const currentSettings = member?.settings
        ? typeof member.settings === "string"
          ? JSON.parse(member.settings)
          : member.settings
        : {};
