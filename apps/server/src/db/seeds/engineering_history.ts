...
export const ADDITIONAL_MESSAGES: SeedMessage[] = [
  // Bug Reports and Fixes
  {
    ...
    type: "text",
    thread: [
      { content: "Steps to reproduce:\\n1. Open two tabs\\n2. Go offline in one\\n3. Other tab still shows as online", sender: 4, type: "text" },
      { content: "Good catch. We need to sync presence across tabs using BroadcastHub", sender: 1, type: "text" },
      { content: "PR with fix: https://github.com/platica/platica/pull/92", sender: 4, type: "system" }
    ],
  ...
  { content: "Deployment failed: TypeError in message parser", sender: 2, type: "system" },
  { content: "Rolling back to previous version", sender: 1, type: "system" },
  { content: "Found the issue - we weren't handling null content in system messages", sender: 2, type: "text" },
  { content: "Fix deployed, all tests passing", sender: 2, type: "system" },
  ...
  // Feature Requests and Planning
  {
    ...
    type: "text",
    thread: [
      { content: "Agreed! Basic requirements:\\n- Emoji picker\\n- Reaction counts\\n- Real-time updates", sender: 3, type: "text" },
      { content: "Let's use emoji-mart for the picker", sender: 4, type: "text" },
      { content: "Good choice. I'll start on the backend schema", sender: 2, type: "text" }
    ],
    attachments: [DEMO_ATTACHMENTS.ui_mockup],
    reactions: [
      { emoji: "✨", users: [1, 2, 3, 4] }, // Everyone likes the idea
  ...
  // Code Review Discussions
  {
    ...
    type: "text",
    thread: [
      { content: "A few concerns:\\n1. N+1 query in thread loading\\n2. Missing index on thread_id\\n3. No pagination for thread replies", sender: 1, type: "text" },
      { content: "Good points. I'll add the index and fix the N+1", sender: 2, type: "text" },
      { content: "Also consider adding a reply count cache", sender: 4, type: "text" },
      { content: "Updated PR: https://github.com/platica/platica/pull/103", sender: 2, type: "system" }
    ],
    attachments: [DEMO_ATTACHMENTS.ui_mockup],
    reactions: [
  ...
  // Architecture Decisions
  {
    ...
    type: "text",
    thread: [
      { content: "Benefits:\\n1. Faster FCP\\n2. Better SEO\\n3. Reduced client bundle", sender: 1, type: "text" },
      { content: "We could use Next.js for this", sender: 3, type: "text" },
      { content: "Or SvelteKit? It has better performance metrics", sender: 4, type: "text" },
      { content: "Let's stick with React ecosystem for now. Next.js is more mature", sender: 1, type: "text" }
    ]
  },
  ...
  // Performance Optimizations
  {
    ...
    type: "text",
    thread: [
      { content: "Main issues:\\n1. Large JS bundle (2.1MB)\\n2. Slow message list rendering\\n3. High memory usage", sender: 4, type: "text" },
      { content: "We can lazy load the emoji picker and file preview components", sender: 2, type: "text" },
      { content: "And implement windowing for long message lists", sender: 3, type: "text" },
      { content: "Created tickets for each item: #234, #235, #236", sender: 4, type: "system" }
    ]
  },
  ...
  // Testing Strategies
  {
    ...
    type: "text",
    thread: [
      { content: "Current coverage is only 45% of critical paths", sender: 3, type: "text" },
      { content: "Let's use Playwright for E2E tests", sender: 2, type: "text" },
      { content: "Good choice. We can run it in GitHub Actions", sender: 1, type: "text" },
      { content: "I'll set up the initial test suite", sender: 3, type: "text" }
    ]
  }
];
...
const CODE_REVIEWS = [
  ...
  type: "text",
  thread: [
    { content: "Looking at it now", sender: 1, type: "text" },
    { content: "Left some comments", sender: 1, type: "text" },
    { content: "Updated PR with fixes", sender: 2, type: "text" },
    { content: "LGTM 👍", sender: 1, type: "text" }
  ],
  attachments: [DEMO_ATTACHMENTS.ui_mockup],
  reactions: [
    { emoji: "🎉", users: [0, 2, 3, 4] }  // Team celebrating the merge
